...
const FEEL_BG: Record<number, string> = {
  1: 'bg-red-50 dark:bg-red-900/10',
  2: 'bg-orange-50 dark:bg-orange-900/10',
  3: 'bg-yellow-50',
  4: 'bg-green-50 dark:bg-green-900/10',
  5: 'bg-teal-50 dark:bg-teal-900/10',
}
        ...
        <div className="px-4 pt-4 pb-3">
          ...
          <div className="flex items-center gap-3 mb-1 flex-wrap">
            ...
            {s.avg_heart_rate != null && (
              <span className="text-xs font-medium text-red-500 bg-red-50 dark:bg-red-900/10 px-2 py-0.5 rounded-full">
            ...
            {s.sync_source === 'strava_webhook' && (
              <span className="flex items-center gap-1 text-xs text-orange-500 dark:text-orange-300 font-medium">
                <StravaLogo /> Strava
              </span>
            )}
              ...
              {([1, 2, 3, 4, 5] as Feel[]).map((v) => (
                <button key={v} onClick={() => setFeel(v)}
                  className={`flex-1 flex flex-col items-center py-2.5 rounded-xl transition-all duration-200 ${
                    feel === v ? 'bg-teal-50 dark:bg-teal-900/10 ring-2 ring-teal-400 shadow-sm scale-105' : 'bg-surface border border-border hover:bg-surface-raised active:scale-95'
                  }`}
          ...
          <div className="flex items-center justify-between">
            {!confirmingDelete && (
              <button onClick={() => setConfirmingDelete(true)} disabled={deleting || saving}
                className="text-sm text-red-500 hover:text-red-700 dark:hover:text-red-300 disabled:opacity-50 px-3 py-2 transition-colors font-medium">
                Delete run
              </button>
            )}
            {confirmingDelete && (
              <div className="flex flex-col gap-1.5">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-red-600 dark:text-red-300 font-medium">Delete this run?</span>
