...
export default function UserRow({ userId, email, role, active, createdAt, isSelf, athletes, linkedAthleteName }: Props) {
  ...
  return (
    <div className={`px-4 py-3 bg-surface ${!active ? 'opacity-50' : ''}`}>
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className={`text-xs font-medium px-2.5 py-1 rounded-lg transition-colors disabled:opacity-50 whitespace-nowrap ${
                  active
                    ? 'bg-red-50 dark:bg-red-900/10 text-red-600 dark:text-red-300 hover:bg-red-100'
                    : 'bg-green-50 dark:bg-green-900/10 text-green-600 dark:text-green-300 hover:bg-green-100'
                }`}
              >
              ...
              <button
                ...
                className="text-xs font-medium px-2.5 py-1 rounded-lg bg-red-50 dark:bg-red-900/10 text-red-600 dark:text-red-300 hover:bg-red-100 transition-colors disabled:opacity-50 whitespace-nowrap"
              >
                {deleting ? '…' : 'Delete'}
              </button>
            </div>
      ...
      {showAthleteSelector && (
        <div className="mt-3 p-3 bg-orange-50 dark:bg-orange-900/10 border border-orange-200 dark:border-orange-400/20 rounded-lg space-y-2">
