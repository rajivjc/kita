      ...
      {chapter.milestones.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-3">
          {chapter.milestones.map((m, i) => (
            <span
              key={i}
              className="inline-flex items-center gap-1 bg-amber-50 dark:bg-amber-900/15 border border-amber-200 dark:border-amber-400/20 text-amber-700 dark:text-amber-300 text-[11px] font-semibold px-2 py-0.5 rounded-full"
            >
              {m.icon ?? '🏆'} {m.label}
            </span>
          ))}
