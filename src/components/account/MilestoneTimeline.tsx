            ...
            <div key={def.id} className="flex gap-3 relative">
              ...
              <div
                className={`relative z-10 w-[30px] h-[30px] rounded-full flex items-center justify-center flex-shrink-0 ${
                  isEarned
                    ? 'bg-amber-100 dark:bg-amber-900/10 ring-2 ring-amber-300'
          ...
          <div className="mt-3 pt-3 border-t border-border-subtle space-y-2">
            {otherEarned.map(m => (
              <Link key={m.id} href={`/milestone/${m.id}`}>
                <div className="flex items-center gap-3 bg-amber-50/50 dark:bg-amber-900/10 hover:bg-amber-50 dark:hover:bg-amber-900/12 rounded-lg px-3 py-2.5 transition-colors">
